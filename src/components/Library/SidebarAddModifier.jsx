...
export default function SidebarAddItemModifier() {
  const dispatch = useDispatch()
  const openSidebarAdd = useSelector((state) => { return state.openSidebarAdd.value })
  let [numOfOption, setNumOfOption] = useState([{}])
  ...
  return (
    <aside className={`${openSidebarAdd ? 'w-[400px] p-6 h-auto' : 'w-[0px] p-0 h-0'} relative z-40 bg-cust-gray block transition-all overflow-x-hidden`}>
      ...
      <div>
        <div className="mb-5">
          <label htmlFor="item Name" className="block text-sm text-cust-gray-darker">Modifier Group Name</label>
          <div className="mt-2">
          ...
          return (
            <>
              <div className="mb-5 flex gap-5">
                <div>
                  <label htmlFor="item Name" className="block text-sm text-cust-gray-darker">Option Name</label>
                  <div className="mt-2">
                    <input type="text" name="item Name" id="item Name" className="block w-full bg-cust-bg-gray border-0 py-1.5 px-3 ring-1 ring-inset ring-gray-400/70 placeholder:text-gray-500/75 focus:ring-2 focus:outline-none focus:border-none focus:ring-inset focus:ring-cust-yellow sm:text-sm sm:leading-6" placeholder="e.g: Dark Chocolate" />
                  </div>
                </div>
                <div>
                  <label htmlFor="item Name" className="block text-sm text-cust-gray-darker">OPtion Price</label>
                  <div className="mt-2">
                    <input type="text" name="item Name" id="item Name" className="block w-full bg-cust-bg-gray border-0 py-1.5 px-3 ring-1 ring-inset ring-gray-400/70 placeholder:text-gray-500/75 focus:ring-2 focus:outline-none focus:border-none focus:ring-inset focus:ring-cust-yellow sm:text-sm sm:leading-6" placeholder="e.g: Dark Chocolate" />
                  </div>
                </div>
        ...
        <div className="mb-5">
          <OutlineButton clickAction={() => { setNumOfOption([...numOfOption, {}]) }}>
            Add option
          </OutlineButton>
        </div>
      </div>
